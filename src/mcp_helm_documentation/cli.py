"""Command-line interface for Helm documentation indexer."""
...
from mcp_helm_documentation.server import DEFAULT_DB_PATH
...
def cmd_stats(args: argparse.Namespace) -> int:
    ...
    if not db_path.exists():
        ...
        logger.info("Run 'helm-docs-index index' to create the index")
        return 1
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="helm-docs-index",
        description="Helm documentation indexer for MCP server",
    )
    parser.add_argument(
        "--database",
        "-d",
    ...
    # Index command
    index_parser = subparsers.add_parser("index", help="Index Helm documentation")
