"""SQLite FTS5 database operations for Helm documentation."""
...
from mcp_helm_documentation.models import Document, SearchResult
...
class DocumentDatabase:
    ...
    def upsert_document(self, doc: Document) -> None:
        ...
        with self._get_connection() as conn:
            conn.execute(
                """
                INSERT INTO documents (path, title, description, section, sidebar_position, url, content)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ...
                """,
                (doc.path, doc.title, doc.description, doc.section, doc.sidebar_position, doc.url, doc.content),
            )
            conn.commit()
