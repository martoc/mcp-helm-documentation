"""Indexer for Helm documentation from helm-www repository."""
...
class HelmDocsIndexer:
    """Indexes Helm documentation from the helm-www GitHub repository."""

    HELM_WWW_REPO = "https://github.com/helm/helm-www.git"
    DOCS_PATH = "docs"
    ...
    def index_from_git(self, branch: str = "main", shallow: bool = True) -> int:
        """Clone helm-www repo and index documentation.
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_path = Path(temp_dir) / "helm-www"
    ...
    def _clone_repository(self, target_path: Path, branch: str, shallow: bool) -> None:
        ...
        if shallow:
            cmd.extend(["--depth", "1"])
        cmd.extend(["--branch", branch, self.HELM_WWW_REPO, str(target_path)])

        logger.info("Cloning helm-www repository...")
