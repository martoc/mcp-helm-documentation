"""FastMCP server for Helm documentation search and retrieval."""
...
DEFAULT_DB_PATH = Path(__file__).parent.parent.parent / "data" / "helm_docs.db"

# Initialise FastMCP server
mcp = FastMCP(name="helm-documentation")
...
def search_documentation(
    ...
    """Search Helm documentation by keyword query.

    Args:
        query: Search terms to find in the documentation. Supports
               full-text search with stemming (e.g., "install" matches
               "installing", "installation").
        section: Optional section to filter results. Valid sections include:
                 'overview', 'intro', 'topics', 'chart_template_guide',
                 'chart_best_practices', 'helm', 'howto', 'sdk', 'plugins',
                 'faq', 'glossary'.
...
def read_documentation(path: str) -> str:
    """Read the full content of a specific Helm documentation page.

    Args:
        path: The relative path to the documentation file (e.g.,
              'intro/quickstart.md' or 'topics/charts.md'). This path
